...
# Создаем локальный лимитер для данного файла
...
def login_user(
    ...
    """
    Аутентификация пользователя через логин (email/username) и пароль.
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    - Если пользователь не существует, создает нового с данными из Google.
    ...
    # Формирование username: используем имя из Google, если нет – берем часть email
    ...
    if not user:
        # Создаем нового пользователя для Google OAuth, без пароля
        user = User(
    ...
    # Генерация JWT-токена для входа
    access_token_expires = timedelta(minutes=ACCESS_TOKEN_EXPIRE_MINUTES)
    payload = {
        "sub": user.email,
        "exp": datetime.utcnow() + access_token_expires,
...
async def github_callback(request: Request, db: Session = Depends(get_db)):
    """
    Обрабатывает ответ от GitHub OAuth, регистрирует или обновляет пользователя,
    ...
    # Ищем пользователя по email или GitHub профилю
