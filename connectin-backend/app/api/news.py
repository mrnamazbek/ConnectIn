...
from app.models.like import NewsLike
from app.models.comment import NewsComment
from app.models.save import SavedNews
...
def like_news(news_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    existing_like = db.query(NewsLike).filter_by(user_id=current_user.id, news_id=news_id).first()
    if existing_like:
        db.delete(existing_like)
        db.commit()
        return {"detail": "Like removed"}
    
    new_like = NewsLike(user_id=current_user.id, news_id=news_id)
    db.add(new_like)
    db.commit()
...
def save_news(news_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    existing_save = db.query(SavedNews).filter_by(user_id=current_user.id, news_id=news_id).first()
    if existing_save:
...
def comment_news(news_id: int, comment_data: CommentCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    new_comment = NewsComment(content=comment_data.content, user_id=current_user.id, news_id=news_id)
    db.add(new_comment)
    db.commit()
