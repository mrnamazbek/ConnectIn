...
@router.post("/", response_model=schemas.todo.TodoInDB)
def create_todo(todo: schemas.todo.TodoCreate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
...
@router.get("/", response_model=list[schemas.todo.TodoInDB])
...
@router.put("/{todo_id}", response_model=schemas.todo.TodoInDB)
def update_todo(todo_id: int, todo: schemas.todo.TodoUpdate, db: Session = Depends(get_db), current_user: models.User = Depends(get_current_user)):
