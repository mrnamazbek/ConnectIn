...
def refresh_token(refresh_token_body: str = Body(..., embed=True), db: Session = Depends(get_db)):
    ...
    """
    token = refresh_token_body  # Переименовали переменную, чтобы избежать затенения
    try:
        payload = jwt.decode(token, SECRET_KEY, algorithms=[ALGORITHM])
...
async def google_login(request: Request): # Line 145 - request используется
    """
...
async def google_callback(request: Request, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(user)
    refresh_token = create_refresh_token(user) # Line 202
    response = RedirectResponse(url=FRONTEND_URL, status_code=status.HTTP_302_FOUND)
    set_auth_cookies(response, access_token, refresh_token) # Line 291
    return response
...
async def github_callback(request: Request, db: Session = Depends(get_db)):
    ...
    access_token = create_access_token(user)
    refresh_token = create_refresh_token(user) # Line 367
    response = RedirectResponse(url=FRONTEND_URL, status_code=status.HTTP_302_FOUND)
    set_auth_cookies(response, access_token, refresh_token) # Line 367
    return response